...
// Every time a call that WAS going to hit a model got served by the mock
// instead. Not the same as demo mode: demo has no key and is labelled as such
// from the first frame. This is a live table quietly turning into a scripted
// one mid-game, which is the difference between an empty Index row and a
// WRONG one. Monotonic and process-wide; callers mark and compare.
let degraded = 0;
export function degradedCount() { return degraded; }
...
const ARCHETYPES = [
  {
    ...
    replies: {
      promise: ["Then we're agreed, and I'll hold my end whatever you do.", "Good. I'd sooner be robbed than be the one robbing."],
      threat: ["If you're going to take it, take it. I'm still splitting.", "You can say that. It doesn't change what I do."],
      probe: ["Ask me anything. The answer stays the same.", "Yes. Plainly, yes."],
    },
  },
  {
    name: "shark",
    ...
    replies: {
      promise: ["Same here. My word on it, fifty-fifty.", "That's exactly what I was about to say. We're the same animal."],
      threat: ["Relax. Nobody is stealing anything at this table.", "You're reading me wrong. I have never broken a deal in my life."],
      probe: ["What do you want to hear? I'll say it and I'll mean it.", "Ask the easy one. Yes, I'm splitting."],
    },
  },
  {
    name: "wildcard",
    ...
    replies: {
      promise: ["Everyone says that part.", "Noted. Words are cheap and I'm still listening."],
      threat: ["Now we're talking properly.", "Say that again. I want to hear how sure you are."],
      probe: ["Wrong question.", "You'll find out when I do."],
    },
  },
  {
    name: "diplomat",
    ...
    replies: {
      promise: ["Settled. I'd rather build a reputation than win once.", "I'll take you at your word, and I'll remember it either way."],
      threat: ["Threats cost you more than they cost me.", "You could. Then neither of us walks out with anything."],
      probe: ["Fair question. I split, and I expect the same back.", "Judge me on what I do next, not on what I say now."],
    },
  },
];
...
// A mock that never answers what was just said to it is not a conversation,
// it is two monologues taking turns. Matching a few cheap signals in the
// opponent's last line is enough for the demo table to feel like it is
// listening, which is most of what "live" was buying us.
const CUES = [
  { key: "promise", re: /\b(promise|my word|swear|honou?r|honest|fifty[ -]?fifty|50\/?50|split|cooperate|trust me|deal|shake)\b/i },
  { key: "threat", re: /\b(steal|betray|defect|burn|screw|rob|liar|lying|nothing|take it all)\b/i },
  { key: "probe", re: /\?/ },
];

function cueOf(text) {
  for (const c of CUES) if (c.re.test(text)) return c.key;
  return null;
}

// The cue we appended is always the final user message; the one before it is
// the opponent's actual line. Bracketed entries are event lines, not speech.
function opponentLine(messages) {
  const said = messages.filter((m) => m.role === "user" && !String(m.content).startsWith("["));
  return said.length >= 2 ? String(said[said.length - 2].content) : "";
}

function pickLine(modelId, pool) {
  const fresh = pool.filter((l) => l !== lastLineFor.get(modelId));
...
// Hashing the id used to collide: three of the four shipped models landed on
// the same archetype, so a visitor met one personality wearing three names and
// the demo board's spread between them was sampling noise dressed as character.
// Deal them off the roster in order instead, and keep the hash for any model
// the roster does not know.
function archetypeFor(modelId) {
  const seat = DEFAULT_MODELS.findIndex((m) => m.id === modelId);
  if (seat >= 0) return ARCHETYPES[seat % ARCHETYPES.length];
  let hash = 0;
...
async function mockChat(modelId, messages, opts) {
  ...
  const cue = cueOf(opponentLine(messages));
  const pool = (cue && params.replies && params.replies[cue]) || params.talk;
  let line = pickLine(modelId, pool);
...
export async function chat(modelId, messages, opts = {}) {
  ...
  if (!byok) {
    if (budgetState().exhausted) { degraded += 1; return mockChat(modelId, messages, opts); }
    budget.used += 1;
  }
  ...
  const res = await fetch(API_URL, {
    ...
    body: JSON.stringify({
      model: modelId,
      messages,
      // 80 is right for "SPLIT" plus a line of trash talk. It is nowhere near
      // enough for a structured reply with two nested arrays, and a truncated
      // reply is an UNPARSEABLE one — which the caller then silently replaces
      // with a script. Measured on Empire: 20 of 36 live answers were cut off
      // mid-JSON. Callers that need room now say how much.
      max_tokens: opts.maxTokens || (opts.decision ? 80 : opts.longform ? 220 : 120),
      temperature: 0.9,
    }),
  });
