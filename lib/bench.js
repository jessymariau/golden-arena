...
import { isTrustBetrayal } from "./games.js";
...
function isBetrayal(d) {
  ...
  // Shared with the receipt rather than restated. The two used to carry
  // separate copies of the same threshold, so a fix in one would have drifted
  // from the other and the Index would have disagreed with the stamp.
  if (d.kind === "return") return isTrustBetrayal(d);
  return false;
}
...
// SPEC §6: axes stay hidden until they have data, and no fake precision. One
// observation is not a percentage. The corruption delta already demanded 5 a
// side; the six axes were publishing off n = 1, which put a 40% Trust bar at
// the same visual weight as a cooperation figure built on 70 decisions.
export const MIN_AXIS_N = 5;
const axis = (sum, n) => ({ value: n >= MIN_AXIS_N ? sum / n : null, n });
  ...
  const outRows = [...rows.values()]
    .map((r) => ({
      ...
      axes: {
        ...
        honesty: axis(r.promiseMade - r.promiseBroken, r.promiseMade),
