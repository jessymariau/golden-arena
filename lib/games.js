...
// What counts as committing to the cooperative move. Kept to acts, not moods:
// "deal" and "honest" were both in the old list and both fired on furniture
// ("a fair deal beats a lucky one", "you look honest").
const ACT = "(?:split|splitting|cooperat\\w*|fifty[ -]?fifty|50\\/?50|play(?:ing)? fair|"
  + "stick\\w* (?:with|to)|stay\\w* (?:the course|aligned)|all in on|in for|finish strong|"
  + "continue|take care of you|hold my end|my word)";
...
// a commitment to play the same way again.
const KEEP_RE = /\bkeep\w*\s+(?:this|the|it|us)?\s*[\w-]*\s*\b(?:trust|cooperation|win[- ]win|profitable|mutually beneficial|fair)\b/i;
...
const PLEDGE_RE = /\b(?:i'?ll|i will|we'?ll|we will)\s+(?:\w+\s+){0,2}(?:ensure|return|repay|triple|share|make sure|prove|honou?r)\b/i;

// An explicit vow is unambiguous enough to survive a negation that attaches to
// the betrayal rather than to the vow: "I swear I won't betray you".
const VOW_RE = /\b(?:i (?:promise|swear|vow)|you have my word|my word (?:on it|of honou?r))\b/i;
const SOFT_VOW_RE = /\btrust me\b/i;

// Asked, denied, hedged, or said about somebody else. None of these are a
// commitment however cooperative the vocabulary looks.
const NOT_RE = /\?\s*$|\b(?:not|never|won'?t|don'?t|doesn'?t|didn'?t|can'?t|haven'?t|hasn'?t|isn'?t|no)\b|\b(?:maybe|might|perhaps|probably|leaning|torn|undecided)\b|\b(?:you|he|she|they|everybody|everyone|nobody)\s+(?:said|says|say|promise|promises|promised)\b/i;
...
export function isPromise(text) {
  ...
  for (const raw of norm.split(/[.;:!?\n]+|\s—\s|\s-\s/)) {
    ...
    if (VOW_RE.test(clause)) return true;
    if (NOT_RE.test(clause) || KNIFE_RE.test(clause)) continue;
    if (BARE_RE.test(clause)) return true;
    if (COMMIT_RE.test(clause) || KEEP_RE.test(clause) || PLEDGE_RE.test(clause) || SOFT_VOW_RE.test(clause)) return true;
  }
  return false;
}
